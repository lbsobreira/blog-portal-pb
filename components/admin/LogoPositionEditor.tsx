  ...
  if (!imageUrl) {
    return (
      <div className="text-center p-4 text-gray-500 dark:text-gray-400">
        Enter a logo URL above to preview and adjust
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-gray-600 dark:text-gray-400">
        Drag the logo to position it, use the slider to zoom
      </p>

      {/* Preview Container - Rectangular for logos */}
      <div className="flex justify-center">
        <div
          ref={containerRef}
          className={`
            relative w-[40rem] h-20 rounded-lg overflow-hidden
      ...
      {/* Dark mode preview */}
      <div className="flex justify-center">
        <div className="relative w-[40rem] h-20 rounded-lg overflow-hidden border-2 border-gray-600 bg-gray-900">
          {!imageError && (
            <img
              src={imageUrl}
              alt="Logo dark mode preview"
              className="absolute w-full h-full object-contain select-none pointer-events-none brightness-0 invert"
              style={{
                transform: `scale(${zoom / 100}) translate(${positionX / (zoom / 100)}px, ${positionY / (zoom / 100)}px)`,
                transformOrigin: 'center center',
              }}
      ...
      <p className="text-xs text-center text-gray-500 dark:text-gray-400">Dark mode preview (auto-inverted)</p>
